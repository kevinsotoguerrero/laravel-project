...
@section('content')
    <div class="bg-gray-900 px-20 py-16 rounded-lg mb-8 relative overflow-hidden">
        ...
        <img src="{{ asset('images/dev.png') }}" class="absolute -right-20 -bottom-28 rounded-full opacity-20">
    </div>
    <div class="px-4">
        ...
        <div class="grid grid-cols-1 gap-4 mb-4">
            @foreach ($posts as $post)
                <a href="{{ route('post', $post->slug) }}" class="bg-gray-100 rounded-lg px-6 py-4">
                    ...
                    <h2 class="text-gray-900 text-lg mt-2">{{ $post->title }}</h2>
                    ...
                    <div class="text-xs text-gray-900 opacity-75 flex items-center gap-1">
                        ...
                        {{-- Si no se trae la informacion de los usuarios desde el controlador la consulta se realiza desde la vista. se puedee ver con el debuger --}}
                        {{ $post->user->name }}
                    </div>
                </a>
            @endforeach
